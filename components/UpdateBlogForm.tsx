...
export default function UpdateBlogForm({ blog, setOpen }: { blog: Partial<IBlog>, setOpen: React.Dispatch<React.SetStateAction<boolean>>}) {
    ...
    const handleUpdateBlog = async (values: BlogFormValues) => {
        ...
        try {
            ...
            const res = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/blog/${blog.id}`, {
                method: "PATCH",
    ...
    return (
        <div className="p-6">
            <Form {...form}>
                <form onSubmit={form.handleSubmit(handleUpdateBlog)} className="space-y-6">
                    <FormField
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                ...
                                <FormControl>
                                    <Textarea {...field} placeholder="blog-content" className="min-h-[180px] max-h-[180px]" />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                    ...
                    {imagePreview && <div className="rounded overflow-hidden border">
                        <Image className="mx-auto" src={imagePreview as string} width={250} height={100} alt={blog?.title as string} />
                    </div>}
